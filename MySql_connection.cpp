        ...
        if( com_type == MYSQL_PROTOCOL_COM_QUERY)
        {
            ...
            else if(local_buf->qInfo.command == TOK_INSERT)
            {
                if(test_api_version(local_buf,PacketNomber))
                {
                    ...
                    else if(local_buf->qInfo.tokCompare("users_conference",  local_buf->qInfo.tableName))
                    {
                        sql_insert_into_users_conference(local_buf,PacketNomber);
                    }
                    else if(local_buf->qInfo.tokCompare("users_dialogs",  local_buf->qInfo.tableName))
                    {
                        sql_insert_into_users_dialogs(local_buf,PacketNomber);
                    }
                    else
                    {
                        Send_Err_Package(SQL_ERR_NOT_EXIST, "Table doesn't exist", PacketNomber+1, local_buf, this);
...
int MySql_connection::sql_insert_into_users_conference(thread_data* local_buf, unsigned int PacketNomber)
{
    TagLoger::log(Log_MySqlServer, 0, " >MySql_connection::users_conference\n");
    ...
    char* pipe_type = pipe_name;

    int r = str_find(pipe_name, '_');
    if(r <= 0)
    {
        Send_Err_Package(SQL_ERR_INVALID_DATA, "field `user_id` is required", PacketNomber+1, local_buf, this);
        return 0;
    ...
    pipe_name[r] = 0;
    pipe_name = pipe_name + r + 1;
 
    std::string sipNumber("000000"); 

    // Определение типа видеорежима
    bool isAudio = strcmp(pipe_type, "audio") == 0;
    bool isVideo = strcmp(pipe_type, "video") == 0;
    // bool isVideo = strcmp(pipe_type, "videoMux") == 0;
    // bool isVideo = strcmp(pipe_type, "videoMux2") == 0;
    ...
    {
        Send_Err_Package(SQL_ERR_INVALID_DATA, "field `name` has invalid value", PacketNomber+1, local_buf, this);
        return 0;
    }
    sipNumber.append(pipe_name);
    TagLoger::log(Log_MySqlServer, 0, " >sipNumber=%s", sipNumber.data());

    char serverPort[] = "7443";
    char serverName[] = "chat.emed24.ru";
    ...
    sprintf(srcHash, "%s_%s_%s_%s", sipNumber.data(), serverName, serverPort, salt);
    ...
    */
    ...
    char* msgData = new char[appConf::instance()->buf_size];
    bzero(msgData, appConf::instance()->buf_size);

    snprintf(msgData, appConf::instance()->buf_size,
            "{\"message\":\"%s\",\"sys\":{\"conference\":true,\"serverName\":\"%s\",\"serverPort\":\"%s\",\"callKey\":\"%s\",\"callPipe\":\"%s\",\"sipNumber\":\"%s\",\"caller_id\":\"%d\",\"pipe_type\":\"%s\",\"pipe_name\":\"%s\"}}",
            local_buf->answer_buf.getData(),
            serverName,
            serverPort,
            callKey,
...
int MySql_connection::sql_insert_into_users_dialogs(thread_data* local_buf, unsigned int PacketNomber)
{
    Send_Err_Package(SQL_ERR_READ_ONLY, "Table `users_dialogs` is not ready yet", PacketNomber+1, local_buf, this);
    return 0;
}
