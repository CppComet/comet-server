...
```
  ...
  * dev_id обязательный параметр это [публичный ключ разработчика](/docs/wiki-md/RU/API/Публичный идентификатор разработчика.md). 
  * user_id не обязательный параметр, это идентификатор пользователя нужен для [авторизации пользователя](/docs/wiki-md/RU/API/Авторизация пользователей.md) на комет сервере
  * user_key не обязательный параметр, это хеш авторизации пользователя нужен для [авторизации пользователя](/docs/wiki-md/RU/API/Авторизация пользователей.md) на комет сервере
...
Пример с online demo [как принять сообщение из канала в JavaScript?](/docs/wiki-md/RU/Примеры/Как принять сообщение из канала в JavaScript?.md)
...
___
Более подробно про механизм авторизации на комет сервере и про личные сообщения смотрите в статье [Авторизация пользователей на комет сервере](/docs/wiki-md/RU/API/Авторизация пользователей.md)
___
...
Основная статья [список пользователей онлайн](/docs/wiki-md/RU/Примеры/Как реализовать  список посетителей обновляющийся на "лету"?.md)
...
___
Логирование проходящих сообщений можно включить запросом CometQL к [таблице pipes_settings](/docs/wiki-md/RU/API/CometQL.md).
___
...
Но в отличии от [CometQL](/docs/wiki-md/RU/API/CometQL.md) запроса эта функция может показывать количество подписчиков только в тех каналах у которых имя начинается с web_ к примеру для канала web_chat_pipe она сработает а для канала chat_pipe не сработает. Это ограничение введено для того что бы была возможность создать такой канал в котором количество подписчиков не сможет узнать кто попало через js api

# Определение статуса авторизации на комет сервере

Основная статья [авторизация на комет сервере](/docs/wiki-md/RU/API/Авторизация пользователей.md)

У комет сервера есть возможность [авторизации пользователей](/docs/wiki-md/RU/API/Авторизация пользователей.md). Авторизация может быть полезна для того что бы была возможность определить кто именно отправил сообщение в канал или для адресной отправки сообщений пользователям.
...
# Дополнительная информация
  ...
  * [CometQL API](/docs/wiki-md/RU/API/CometQL.md)
